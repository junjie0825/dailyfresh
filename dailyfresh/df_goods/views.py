...
def list(request, tid, pindex, sort):
    ...
    if sort == '1':
        goods_list = GoodsInfo.objects.filter(gtype_id__exact=int(tid)).order_by('-id')
    elif sort == '2':
        goods_list = GoodsInfo.objects.filter(gtype_id__exact=int(tid)).order_by('-gprice')
    elif sort == '3':
        goods_list = GoodsInfo.objects.filter(gtype_id__exact=int(tid)).order_by('-gclick')
